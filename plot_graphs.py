...
for c in range(0, total_samples):
    x.append(mean)
# Plotting a line with the mean
plt.plot(x, y, color="red", label=f"Mean: {mean}")

x = []
for c in range(0, total_samples):
    x.append(median)
# Plotting a line with the median
...
plt.style.use("bmh")
plt.errorbar("Samples", mean, yerr=error, color="blue",
             fmt='o', label=f"t = ({mean} +/- {error}) ms")
